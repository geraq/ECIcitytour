...
Agencies:
A (frequent flyer) = 3 times in a row --> 35% off the price of the third flight 
...
function getFitness!(pop::AbstractArray{Int}, distances::Vector{Float64}, 
    prices::Vector{Float64}, costs::AbstractArray{Float64})::Vector{Float64}  
  getCosts!(pop, distances, prices, costs)      
...
function getCosts!(pop::AbstractArray{Int}, distances::Vector{Float64}, prices::Vector{Float64}, 
    costs::AbstractArray{Float64})
...
function makeNewPop!(pop::AbstractArray{Int}, newPop::AbstractArray{Int}, probs::Vector{Float64}, pCross::Float64, pMut::Float64, crossOverMethod::CrossOverMethod)  
  M = size(pop, 1)
...
function keepBestIndividual!(best::Vector{Int}, bestFitness::Float64, newPop::AbstractArray{Int}, newPopFitness::Vector{Float64})
  toReplace = indmin(newPopFitness)
  newPop[toReplace, :] = best
  newPopFitness[toReplace] = bestFitness
end

function elitism(pop::AbstractArray{Int}, fitness::Vector{Float64}, newPop::AbstractArray{Int}, newPopFitness::Vector{Float64})
  M = size(pop, 1)
  fullpop = [pop ; newPop]
...
originalSolution = convert(AbstractArray{Int}, [mapping[c] for c in cityPairs[:agency]]')

pop = SharedArray(rand(1:nAgencies, popSize, nCities))
...
costs = SharedArray(repmat(prices', popSize, 1)) 
fitness = getFitness!(pop, distances, prices, costs)
newPop = SharedArray(zeros(pop))
while gen <= maxGen  
  ...
  makeNewPop!(pop, newPop, probs, pCross, pMut, crossOverMethod)
  newPopFitness = getFitness!(newPop, distances, prices, costs)
  if USE_ELITISM
...
cost = zeros(champions[bestChampion:bestChampion,:])
getCosts!(champions[bestChampion:bestChampion,:], distances, prices, cost)
